...
$fecha_inicial_prueba = date_create($_POST['fecha-inicio']);
$fecha_final_prueba = date_Create($_POST['fecha-final']);

$comparacion_ultima = date_diff($fecha_inicial_prueba,$fecha_final_prueba,false);
$nuevo=array();
foreach ($comparacion_ultima as $datos) {

    $nuevo[] =$datos; 
} 
$comparar_invert_ultimo=$nuevo[10];
...
if (!$comparacion_invert_inicial == 1 && !$comparacion_invert_final==1 && !$comparar_invert_ultimo==1 ) {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO tbl_permiso_calamidad(perc_fecha_solicitud,perc_fecha_inicio,perc_fecha_fin,perc_dia_total,perc_adjunto,perc_estado,id_persona_c,perc_permiso) VALUES (?,?,?,?,?,?,?,?)");
        $stmt->bind_param("sssissis",$fecha_solicitud, $fecha_inicio, $fecha_final, $final_total, $link, $estado, $cliente,$permiso);
        $stmt->execute();
        if($stmt->affected_rows > 0) {
             $respuesta = array(
                  'respuesta' => 'correcto calamidad',
